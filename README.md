...
* **monitors**
  ...
  - "web-operations": this allows for simulating user interactions on the website before the scraping process, like for example logging in or selecting certain filters on a product website. Currently it is only available at the very beginning, not when accessing urls found in "href". It is defined by an array of objects which all have a "type" key. Here are the types and other keys expected in each type:
    - "wait": has key "time" with an int which represents wait time in seconds.
    - "scroll": has key "scrolls" for an int which represents how often should be scrolled to the bottom of the page. It will scroll to the bottom of the page, check if the page just got longer and continue this as often as specified. 0 will make it scroll indefinitely until no new content has been loaded.
    - "click": has key "css-selector" which works like the other css selectors. It is being used to identify which element should be clicked.
    - "enter-text": has the keys "css-selector" which works as described for "click", "text", a string which specifies the text to be entered and optionally "submit", a boolean which specifies if ENTER should be hit after the text has been entered.
